...
class Controller {
    ...
    Controller() {
        ...
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
                state.map[i][j] = 0;//rand.nextInt(5) + 1;
            }
        }
        try {
...
/*                    if (x == 254 && y == 122) {
                        int t1 = aim.getRGB(i, j);
                        int t2 = tot.getRGB(x + i, y + j);
                        System.out.println(t1 + " " + t2);
//                        System.out.println(((t1 >> 16) & 0xff) + " " + ((t2 >> 16) & 0xff) + " " + ((t1 >> 8) & 0xff) + " " + ((t2 >> 8) & 0xff) + " " + (t1 & 0xff) + " " + (t2 & 0xff));
